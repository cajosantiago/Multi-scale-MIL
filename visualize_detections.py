...
def Compute_Heatmaps_patches(model: torch.nn.Module,
    ...
    if args.mil_type == 'pyramidal_mil' and args.pooling_type in ['gated-attention', 'attention', 'pma']:
...
def Visualize_ROI_Eval(model,
    ...
    reverse_transform = transforms.Compose([
        transforms.Normalize((-args.mean / args.std, -args.mean / args.std, -args.mean / args.std),
    ...
    for idx, data in enumerate(tqdm(dataloader)):
        ...
        if img_path in df['img_path'].values:
            ...
            boxes = df[df['img_path'] == img_path]['boxes'].values[
                0]  # Retrieve gt bounding boxes for this image from dataframe
            ...
            bag_prob, heatmaps = Compute_Heatmaps_patches(model, inputs, bag_coords, bag_info, False, seg_mask, device,
                                                          args)
            ...
            if args.mil_type == 'pyramidal_mil':
                ...
                for idx, (scale, data) in enumerate(heatmaps.items()):
                    ...
                    if scale == "aggregated":
                        ...
                        attention_vis = ShowVis(data["heatmap"], image_rgb.permute(1, 2, 0), data["pred_bboxes"],
                                                axs[plot_idx][idx + 1], args)
                        axs[plot_idx][idx + 1].set_title(f"Aggregated Heatmap (IoU: {iou_score * 100:.2f})",
                                                         fontsize=16)
                ...
                ShowVis(heatmaps['heatmap'], image_rgb.permute(1, 2, 0), heatmaps['pred_bboxes'], axs[plot_idx][1],
                        args)
                axs[plot_idx][1].set_title(f"Heatmap (IoU: {iou_score * 100:.2f})", fontsize=16)
                axs[plot_idx][1].axis('off')

                # Add colorbar on the last heatmap subplot
                cbar = fig.colorbar(plt.cm.ScalarMappable(cmap='jet'), ax=axs[args.visualize_num_images - 1][1],
            ...
            del img, image_rgb, seg_mask, heatmaps;
            clear_memory()
...
def run_roi_eval(directory, args, device):
    ...
    roi_dir = os.path.join(directory, 'roi_visualization_new', args.dataset, args.roi_eval_set, args.roi_eval_scheme,
                           f'{args.iou_method}_threshold_{args.iou_threshold}')
    ...
    for idx, data in enumerate(tqdm(dataloader)):
        ...
        bag_prob, heatmaps = Compute_Heatmaps_patches(model, inputs, bag_coords, bag_info,
                                                      total_num_imgs < args.visualize_num_images, seg_mask, device,
                                                      args)
        ...
        if args.mil_type == 'pyramidal_mil':
            ...
            for scale, heatmap in heatmaps.items():
                scores[scale], iou_scores, false_positives[scale], true_positives[scale] = evaluate_metrics(boxes,
                                                                                                            heatmap[
                                                                                                                'pred_bboxes'],
                                                                                                            scores[
                                                                                                                scale],
            ...
            scores, iou_scores, false_positives, true_positives = evaluate_metrics(boxes, heatmaps['pred_bboxes'],
                                                                                   scores, false_positives,
                                                                                   true_positives, args.iou_threshold,
                                                                                   args.iou_method)
    ...
    del indices, false_positives, true_positives, recall, precision, average_precision_area, average_precision_11points;
    clear_memory()
