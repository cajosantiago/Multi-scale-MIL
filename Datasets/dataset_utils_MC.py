...
from .dataset_concepts_MC import Generic_MIL_Dataset_Detection, collate_MIL_patches_detection, Generic_MIL_Dataset, collate_MIL_patches, BagDataset, collate_patch_features, Generic_MIL_Dataset_Multiview, group_views_by_breast
...
# probably the function to be changed
...
def MIL_dataloader(split_df, split, args):
    """
    Creates and returns a PyTorch DataLoader for MIL tasks, handling both online feature extraction and pre-extracted features,
    as well as optional ROI evaluation and data augmentation.
    ...
    elif args.label.lower() == 'breast_density':
        label_dict ={
            'DENSITY A': 0,
            'DENSITY B': 1,
            'DENSITY C': 2,
    ...
    if args.roi_eval: 
        ...
        # Multi-view MIL dataset
        grouped_laterality_df = group_views_by_breast(split_df, label_dict=label_dict, args=args)
        split_dataset = Generic_MIL_Dataset_Multiview(args=args, df=grouped_laterality_df, transform=tfm)
    else:
