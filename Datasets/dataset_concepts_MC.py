...
class Generic_MIL_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
        'x': x,
        'y': torch.tensor(data['label_num'], dtype=torch.long)
        }

def group_views_by_breast(df, label_dict, args):
    """groups the 2 views (CC ans MLO) by breast (L or R) for each patient"""
    
    if args.label.lower() == 'breast_density':
        grouped = df.groupby(['patient_id', 'laterality']).apply(
            lambda x: {
            'paths' : x['image_id'].tolist(),
            'patient_id' : str(x['patient_id'].iloc[0]),
            'laterality' : x['laterality'].iloc[0],
            'views' : x['view'].tolist(),
            'breast_density_str' : x['breast_density'].iloc[0],
            'breast_density' : label_dict[x['breast_density'].iloc[0]]
        }
        ).reset_index(name = 'breast_data')

    if args.label.lower() == 'breast_birads':
        grouped = df.groupby(['patient_id', 'laterality']).apply(
            lambda x: {
            'paths' : x['image_id'].tolist(),
            'patient_id' : str(x['patient_id'].iloc[0]),
            'laterality' : x['laterality'].iloc[0],
            'views' : x['view'].tolist(),
            'breast_birads_str' : x['breast_birads'].iloc[0],
            'breast_birads' : label_dict[x['breast_birads'].iloc[0]]
        }
        ).reset_index(name = 'breast_data')
    return grouped

class Generic_MIL_Dataset_Multiview(Generic_MIL_Dataset):
    def __getitem__(self, idx):

        data = self.df.iloc[idx]
        if 'breast_data' in data and isinstance(data['breast_data'], dict):
            return self.__getmultiviewitem__(data['breast_data'])
        else:
            return super().__getitem__(idx)

    def __getmultiviewitem__(self, breast_data):
        if self.transform is not None: 
            # online features
            print("online multiview")
            return self.__getmultiview_online__(breast_data)
        else:
            # offline features
            return self.__getmultiview_offline__(breast_data)

    def __getmultiview_online__(self, breast_data):
        """elif self.label.lower() == 'breast_birads':
            return { 
                'x': combined_patches,
                'y' : torch.tensor(breast_data['breast_birads'], dtype=torch.long)
                }
        else:
            print("Label not recognized for multiview dataset")"""

        all_patches = []

        for img_path in breast_data['paths']:
            bag_dir = self.dir_path / breast_data['patient_id'] / img_path

            img = Image.open(bag_dir).convert('RGB')
            x, bag_coords, padding = self.transform(img)
            all_patches.append(x)

        # concatenate patches from both views in one tensor

        combined_patches = torch.cat(all_patches, dim=0)
        
        return { 
            'x': combined_patches,
            'y': torch.tensor(breast_data[self.label.lower()], dtype=torch.long)
            }
    

    
    def __getmultiview_offline__(self, breast_data):
        ...
        for img_path in breast_data['paths']:
            ...
            if self.multi_scale_model is None: 
                ...
                # Multi-scale pyramid: load features for all scales
                features = {}
                #bag_dir_small = self.dir_path / f'patch_size-{self.scales[0]}' / breast_data['patient_id'] / img_path
                bag_dir_small = self.dir_path / f'patch_size-{128}' / breast_data['patient_id'] / img_path
                x_small  = self.load_data(bag_dir_small) 
                #features[self.scales[0]] = x_small
                features[128] = x_small

                #bag_dir_medium = self.dir_path / f'patch_size-{self.scales[1]}' / breast_data['patient_id'] / img_path
                bag_dir_medium = self.dir_path / f'patch_size-{256}' / breast_data['patient_id'] / img_path
                x_medium = self.load_data(bag_dir_medium)
                #features[self.scales[1]] = x_medium
                features[256] = x_medium

                #bag_dir_large = self.dir_path / f'patch_size-{self.scales[2]}' / breast_data['patient_id'] / img_path
                bag_dir_large = self.dir_path / f'patch_size-{384}' / breast_data['patient_id'] / img_path
                x_large = self.load_data(bag_dir_large)
                #features[self.scales[2]] = x_large
                features[384] = x_large
                ...
                dir_path = self.dir_path / breast_data['patient_id'] / img_path
        ...
        # concatenate features from both views in one tensor/list/dict depending on the model type
        if self.multi_scale_model is None: # single-scale > concatenate : all_features = [tensor_MLO, tensor_CC]
            combined_features = torch.cat(all_features, dim=0)

        elif self.multi_scale_model == 'msp': # multi-scale patch-based > concatenate each scale separately
        # all_features = [features_MLO, features_CC] 
        # with features_MLO = {scale1: tensor1, scale2: tensor2, scale3: tensor3}
            combined_features = {}

            for scale in [128, 256, 384]: #self.scales:
                scale_features = [features[scale] for features in all_features]
                combined_features[scale] = torch.cat(scale_features, dim=0)

        elif self.multi_scale_model in ['fpn', 'backbone_pyramid']: # FPN-based > concatenate each feature level separately
        # all_features = [features_MLO, features_CC]
        # with features_MLO = [C4_features, C5_features]

            combined_features = []
            for level in range(len(all_features[0])):
                level_features = [features[level] for features in all_features] # C4_combined = [C4_MLO, C4_CC]
                combined_features.append(torch.cat(level_features, dim=0)) # combined_features = [C4_combined, C5_combined]
...
def collate_MIL_patches(batch):
    ...
    return {
        'x': x, 
        'y': torch.from_numpy(np.array([item["y"] for item in batch], dtype=np.int64))
    }
...
class Generic_MIL_Dataset_Detection(Dataset):
    def __init__(self, args, df, transform=None):
        ...
        if 'label_num' not in self.df.columns:
            self.df['label_num'] = self.df[self.label].map(args.label_dict)
    ...
    def _generate_image_dict(self):
        ...
        for idx, row in self.df.iterrows():
            ...
            if any(boxes): 
                ...
                image_dict["labels"].append(row['label_num']) # multi class
