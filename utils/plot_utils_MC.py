...
"""check ROC_curves_multiclass - A REVOIR"""
...
def ROC_curves_multiclass(test_targs, test_probs, checkpoint_name, output_dir, model_ema=False, class_names=None):
    """A REVOIR"""
    n_classes = test_probs.shape[1]
    if class_names is None:
        class_names = [f"BI-RADS {i+1}" for i in range(n_classes)]
    
    y_true_bin = label_binarize(test_targs, classes=np.arange(n_classes))
    plt.figure(figsize=(8, 6))
    for i in range(n_classes):
        fpr, tpr, _ = metrics.roc_curve(y_true_bin[:, i], test_probs[:, i])
        roc_auc = metrics.auc(fpr, tpr)
    ...
    plt.legend(loc="lower right")
    plt.tight_layout()
    model = '_model_ema' if model_ema else ''
    plt.savefig(output_dir / f'{checkpoint_name}-roc_curve_OVR{model}.png', bbox_inches='tight')
    plt.clf()


def print_misclassified_patients(targets, preds, id, split_name = 'val'):
# TO B EPUT INTO VALID_FN 
    missclassified_mask = targets!=preds
    missclassified_indices = np.where(missclassified_mask)[0]

    num_total = len(targets)
    num_misclass = len(missclassified_indices)

    if num_missclass ==0:
        print("no missclassified")
        return
    
    for idx in missclassified_indices:
        print(dataset.df.iloc[idx][['patient_id', 'image_id', 'label', 'label_num']])
        print(id[idx])
